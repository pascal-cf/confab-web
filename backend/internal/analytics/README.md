...
| `trends.go` | `Store.GetTrends` -- single-user, date-range analytics dashboard. Runs six parallel aggregation queries (overview+activity, tokens, tools, agents+skills, top sessions, providers-present). The overview+activity path groups by `(session_date, session_type)` so `DailySessionCount.PerProvider` carries per-canonical-provider counts for the stacked-bar chart (CF-444); legacy `Claude Code` folds into `claude-code` at the Scan site. The tokens path also runs a second `GROUP BY session_type` query via `aggregateTokensPerProvider` for the CF-435 per-provider breakdown. `resolveProviderFilter` expands canonical provider values with legacy aliases and defaults to `models.AllowedProviders` so the `session_type = ANY` clause is always present (guards CF-352-style silent omission). |
| `trends_types.go` | Request/response types for the trends API (`TrendsRequest` with `Providers` filter, `TrendsResponse` with top-level `ProvidersPresent`, `TrendsCards`, daily breakdown types, plus `TrendsTokensPerProvider` + `TrendsTokensCard.PerProvider` map for CF-435 and `DailySessionCount.PerProvider` map for CF-444). |
